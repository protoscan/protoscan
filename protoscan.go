...
type Protoscan struct {
	...
	tokens    []byte    // Last tokens returned by Split.
	indexes   []int     // Last tokens indexes.
	gaps      []byte    // Last skipped bytes returned by Split. Gaps intend to holds buffered and unprocessed bytes for example if EOF occurs.
...
//
// The gaps intend to returns buffered and unprocessed bytes for example if EOF occurs.
type SplitFunc func(data []byte, atEOF bool) (hint int, advance int, tokens []byte, indexes []int, gaps []byte, err error)
...
// Tokens returns the most recent tokens generated by a call to Scan.
// The underlying array may point to data that will be overwritten
// by a subsequent call to Scan. It does no allocation.
func (s Protoscan) Tokens() []byte { return s.tokens }

// Indexes returns the most recent indexes for a tokens generated by a call to Scan.
func (s Protoscan) Indexes() []int { return s.indexes }

// Gaps intend to holds buffered and unprocessed bytes for example if EOF occurs.
func (s Protoscan) Gaps() []byte { return s.gaps }
...
func (s *Protoscan) Scan() bool {
	...
	for {
		hint, advance, tokens, indexes, gaps, err := s.Split(s.Buffer[s.start:s.end], s.err == io.EOF)
		s.tokens = tokens
		s.indexes = indexes
		s.gaps = gaps
		if err != nil {
